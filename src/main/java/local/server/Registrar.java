...
public class Registrar extends ServerEngine {
    ...
    public void processRequestToLocalServer(Message msg) {
        ...
        if (server_profile.is_registrar && msg.isRegister()) {
         ...
         {  // check message authentication
            ...
            if (server_profile.do_authentication) {  // add Authentication-Info header field
    ...
    protected Message updateRegistration(Message msg) {
        ...
        if (!msg.hasContactHeader()) {  //printLog("ContactHeader missed: message discarded",Log.LEVEL_HIGH);
            //int result=484;
        ...
        location_service.sync(); //写文件进user.db
        return resp;
    }
