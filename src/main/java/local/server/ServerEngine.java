...
public abstract class ServerEngine implements SipProviderListener {
    ...
    public ServerEngine(SipProvider provider, ServerProfile profile) {
        ...
        for (int i = 0; i < LOCATION_SERVICES.length; i++)
            if (LOCATION_SERVICES[i].equalsIgnoreCase(profile.location_service)) {
        ...
        } else { // remove all expired contacts
            boolean changed = false;
            for (Enumeration u = location_service.getUsers(); u.hasMoreElements(); ) {
                String user = (String) u.nextElement();
                for (Enumeration c = location_service.getUserContactURLs(user); c.hasMoreElements(); ) {
                    String contact = (String) c.nextElement();
                    if ((changed = location_service.isUserContactExpired(user, contact)) == true)
                        location_service.removeUserContact(user, contact);
                }
        ...
        if (server_profile.do_authentication || server_profile.do_proxy_authentication) {  // first, init the proper authentication service
            ...
            for (int i = 0; i < AUTHENTICATION_SERVICES.length; i++)
                if (AUTHENTICATION_SERVICES[i].equalsIgnoreCase(profile.authentication_service)) {
            ...
            if (authentication_service == null)
                authentication_service = new AuthenticationServiceImpl(server_profile.authentication_db);
            ...
            for (int i = 0; i < AUTHENTICATION_SCHEMES.length; i++)
                if (AUTHENTICATION_SCHEMES[i].equalsIgnoreCase(profile.authentication_scheme)) {
    ...
    public void onReceivedMessage(SipProvider provider, Message msg) {
        ...
        if (msg.isRegister()){
            ...
            String testCaseId = RcsServer.getMap().get("testCaseId"); //用例编号
            // String userAccount = RcsServer.getMap().get("userAccount"); //账号
            if (redisTemplate == null){
                redisTemplate = (RedisTemplate<String,Object>)SpringContextUtils.getContext().getBean("redisTemplate");
            }
            List<CaseProcess> caseProcesses = (List<CaseProcess>)redisTemplate.opsForValue().get(testCaseId); //用例编号取流程
            Set<String> keys = redisTemplate.keys("*"+msg.getFromHeader().getNameAddress().getAddress().getUserName()+"*");//模糊匹配
            if(!keys.isEmpty() && keys.size() == 1){
                //表示存在这个key
                Iterator<String> iterator = keys.iterator();
                String userAccount = iterator.next();
                //得到顺序
                Integer order = (Integer) redisTemplate.opsForValue().get(userAccount);
                if (order == null){
                    //表示没有进行过
                    order = 1;
                }
                for (int i = order; i < caseProcesses.size() ; i++) {
                    CaseProcess caseProcess = caseProcesses.get(i); //得到流程1（服务器发送的）
                    //得到模板,并且替换，发送
                    //测试Message，没有模板直接构建
                    //改变requestLine
                    ...
                    if (targets.isEmpty()){
                        //用户未注册，返回404
                        sip_provider.sendMessage(MessageFactory.createResponse(msg, 404, null, null));
                    }else {
                        sendRespMsg(targets,msg);
                    }

                    //判断下一条的顺序，如果还是服务器到客户端的，则
                    for (int j = i+1; j < caseProcesses.size(); j++) {
                        CaseProcess nextCaseProcess = caseProcesses.get(j);
                        if ("服务器到终端".equals(nextCaseProcess.getOrientation())){
                            //得到模板，替换数据，一起发送
                            //Message，没有模板直接发送202
                            Message response = MessageFactory.createResponse(msg, 202, SipResponses.reasonOf(202), RandomUtil.generate(), null, null, null);
                            response.setDateHeader(new DateHeader(new Date())); //设置Data域
                            response.setServerHeader(new ServerHeader(Host.SERVER_ID)); //设置server域
                            ViaHeader viaHeader = response.getViaHeader();
                            response.removeViaHeader();
                            viaHeader.setRport(msg.getRemotePort());
                            viaHeader.setReceived(msg.getRemoteAddress());
                            response.addViaHeader(viaHeader);
                            sip_provider.sendMessage(response);
                        }else {
                            order = j;
                            redisTemplate.opsForValue().set(userAccount,order);
                            break;
                        }
                ...
                System.out.println("这个账号不用于这个测试用例");
            }
        }


//        if (caseProcesses != null && caseProcesses.size() != 0){
//            for (int i = 0; i < caseProcesses.size(); i++) {
//                Integer order = (Integer)redisTemplate.opsForValue().get(userAccount);
//                if (order == null){
//                    CaseProcess caseProcess = caseProcesses.get(0);
//                    //组装模板消息
//                    redisTemplate.opsForValue().set(userAccount,caseProcess.getSequenceNumber() + 1);
//                }else {
//                    for (int j = i;j < caseProcesses.size();j++){
//                        if ("服务器到终端".equals(caseProcesses.get(j).getOrientation())){
//                            //表示同时进行
//
//                        }
//                    }
//                }
//                redisTemplate.opsForValue().set(userAccount,caseProcesses.get(i).getSequenceNumber() + 1);
//            }
//        }
        //接收消息
//        if (msg.isRequest()){
//            if (msg.isMessage()){
//                //构造202给A
//                Message response = MessageFactory.createResponse(msg, 202, SipResponses.reasonOf(202), null);
//
//                //构造Message返回给B
//
//            }
...
//                request.removeRequestLine(); //第一行不要了
//                // SipURL target_url = new SipURL((String) (targets.elementAt(i)));  需要登录之后才能知道的
//                request.setRequestLine(new RequestLine(msg.getRequestLine().getMethod(), ""));
//                String proto = null;
//                if (request.hasRouteHeader()) {
//                    SipURL route = request.getRouteHeader().getNameAddress().getAddress();
//                    if (route.hasTransport()) proto = route.getTransport();
//                } else proto = request.getRequestLine().getAddress().getTransport();
...
//                if (sip_provider.isRportSet()) via.setRport(); //设置via的rport值
//                String branch = sip_provider.pickBranch(request); //设置via的branch值
//                via.setBranch(branch);
//                request.addViaHeader(via);
//                MaxForwardsHeader maxfwd = request.getMaxForwardsHeader();
//                if (maxfwd != null) maxfwd.decrement();
//                else maxfwd = new MaxForwardsHeader(SipStack.max_forwards);
//                request.setMaxForwardsHeader(maxfwd);
//                msg.rfc2543RouteAdapt();
//                sip_provider.sendMessage(request);
//            }
//        }else {
//            if (msg.isResponse()) {   //被叫回给主叫
//                printLog("message is a response", Log.LEVEL_LOW);
//                //服务器回给A
//                processResponse(msg);
//            } else
//                printWarning("received message is not recognized as a request nor a response: discarded", Log.LEVEL_HIGH);
//        }
...
//            if (err_resp != null) {  // for non-ACK requests respond with an error message
...
//                    if (err_resp != null) {  //sip_provider.sendMessage(err_resp);
...
//        }
     ...
     */
    protected boolean isResponsibleFor(String domain, int port) {  // check port
     ...
     */
    protected boolean isResponsibleFor(SipURL url) {  //return isResponsibleFor(url.getHost(),url.getPort());
        ...
        if (err_code == 0 && server_profile.loop_detection) {
            ...
            if (!msg.hasRouteHeader()) {
                ...
                for (int i = 0; i < v.size(); i++) {
                    ...
                    if (sip_provider.getViaAddress().equals(vh.getHost()) && sip_provider.getPort() == vh.getPort()) {  // possible loop
   ...
   {  String branch=sip_provider.pickBranch(msg);
    ...
    private void sendRespMsg(Vector targets,Message msg){
        ...
        for (int i = 0; i < targets.size(); i++) {
            ...
            fromHeader.setParameter("tag",RandomUtil.generate());
            request.setFromHeader(fromHeader);
            request.setUserAgentHeader(new UserAgentHeader(Host.SERVER_ID));
            request.setDateHeader(new DateHeader(new Date()));
            ViaHeader viaHeader = request.getViaHeader();
            viaHeader.removeParameter("rport");
            viaHeader.removeParameter("received");
            request.removeViaHeader();
            request.addViaHeader(viaHeader); //去掉report和received
            request.removeCallIdHeader();
            request.setCallIdHeader(new CallIdHeader(RandomUtil.generate()+"@"+Host.HOST));
            MaxForwardsHeader maxfwd = msg.getMaxForwardsHeader();
            if (maxfwd != null) maxfwd.decrement();
            else maxfwd = new MaxForwardsHeader(SipStack.max_forwards);
            request.setMaxForwardsHeader(maxfwd); //MaxForwardsHeader减1
            sip_provider.sendMessage(request);
        }
    }
}
