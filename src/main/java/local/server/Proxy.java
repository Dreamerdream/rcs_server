...
public class Proxy extends Registrar {
    ...
    public Proxy(SipProvider provider, ServerProfile server_profile) {
        ...
        if (server_profile.call_log)
            call_logger = new CallLoggerImpl(SipStack.log_path + "//" + provider.getViaAddress() + "." + provider.getPort() + "_calls.log");
    }
    ...
    public void processRequestToLocalServer(Message msg) {
        ...
        } else if (!msg.isAck()) {  // send a stateless error response
    ...
    public void processRequestToLocalUser(Message msg) {
      ...
      {  Message err_resp=as.authenticateProxyRequest(msg);  
        ...
        if (targets.isEmpty()) {  // prefix-based forwarding
            SipURL request_uri = msg.getRequestLine().getAddress();
            SipURL new_target = null;
            if (isResponsibleFor(msg.getFromHeader().getNameAddress().getAddress()))
                new_target = getAuthPrefixBasedProxyingTarget(request_uri);
    ...
    public void processRequestToRemoteUA(Message msg) {
        ...
        if (!server_profile.is_open_proxy) {  // check whether the caller or callee is a local user
      ...
      {  Message err_resp=as.authenticateProxyRequest(msg);  
        ...
        SipURL nexthop = null;
        if (isResponsibleFor(msg.getFromHeader().getNameAddress().getAddress()))
            nexthop = getAuthDomainBasedProxyingTarget(request_uri);
