...
public class SessionBorderController extends Proxy {
     ...
     */
    private void init(ExtendedSipProvider sip_provider, ServerProfile server_profile, SessionBorderControllerProfile sbc_profile) {  // init
        ...
        if (sbc_profile.media_addr == null || sbc_profile.media_addr.equals("0.0.0.0"))
            sbc_profile.media_addr = sip_provider.getViaAddress(); //本机地址
...
//        if (msg.isInvite()) {
//            printLog("CallServerExAgent.create start", Log.LEVEL_MEDIUM);
//            Map<String, String> params = new HashMap<String, String>();
//            params.put("RseEx.SDP", PARAMS);
//            Common.Holder.Long callId = new Common.Holder.Long();
//            boolean result = CallServerExAgent.create(mCallServerExAgent, "[username:12@sample.cloud.justalk.com]", params, callId, null);
//            printLog("CallServerExAgent.create " + (result ? "success" : "fail"), Log.LEVEL_MEDIUM);
//        }
        msg = SipMangler.unmangleRequestLine(msg);
        if (isResponsibleFor(msg) && !SipMangler.isRequestLineMangled(msg)) {
   ...
   {  printLog("inside processRequestToRemoteUA(msg)",Log.LEVEL_MEDIUM);
    ...
    protected Message updateProxyingRequest(Message req) {
        ...
        if (req.hasContactHeader())
            req = SipMangler.mangleContact(req, sip_provider.getViaAddress(), sip_provider.getPort());
        return req;
    }
    ...
    protected Message updateProxyingResponse(Message resp) {
        ...
        if (resp.hasContactHeader()) {
            if (resp.getCSeqHeader().getMethod().equalsIgnoreCase(SipMethods.REGISTER))
                resp = SipMangler.unmangleContact(resp);
            else resp = SipMangler.mangleContact(resp, sip_provider.getViaAddress(), sip_provider.getPort());
        }
        // update the SIP keep alive daemons
        if (keepalive_daemons != null && resp.getCSeqHeader().getMethod().equalsIgnoreCase(SipMethods.REGISTER))
            updateKeepAlive(resp);
        return resp;
    }
   ...
   {  printLog("inside mangleContact()",Log.LEVEL_MEDIUM);
    ...
    private static Common.ObjectAgent mCallServerExAgent;
    ...
    public static void init(){
        ...
        int first_port = 0;
        int last_port = 0;
        SipStack.init(file); //读取配置文件,配置sip的属性
        ServerProfile server_profile = new ServerProfile(file); //配置服务器属性？
        SessionBorderControllerProfile sbc_profile = new SessionBorderControllerProfile(file); //读属性，加端口？
        ...
        // keepalive_aggressive:false
        // keepalive_aggressive_time:0
        long keepalive_aggressive_time = (sbc_profile.keepalive_aggressive) ? sbc_profile.keepalive_time : 0;
        //ExtendedSipProvider extends SipProvider
        ...
        SessionBorderController controller = new SessionBorderController(extended_provider, server_profile, sbc_profile);
...
//        RcsServer rcsServer = new RcsServer();
//        rcsServer.init();
//        controller.sip_provider.addSelectiveListener(SipProvider.INVITE,new MyInviteSipProviderListener());
//        controller.sip_provider.addSelectiveListener(SipProvider.REGISTER,new MyRegisterSipProviderListener(rcsServer));
//        controller.sip_provider.addSelectiveListener(SipProvider.SUBSCRIBE,new MySubscribeSipProviderLister());
//        controller.sip_provider.addSelectiveListener(SipProvider.MESSAGE,new MyMessageSipProviderListener());
//        controller.sip_provider.addSelectiveListener(SipProvider.OPTIONS,new MyOptionsSipProviderListener());

//      if (!Common.Util.loadLibrary("CommonJNI")) {
//          return;
//      }
//      try {
//         //jni.......
//    	  Common.RouterAgent driver = new Common.RouterAgent(null);
//    	  Common.Application application = new Common.Application("client", "config/justalk_server.cfg",driver,null);
//    	  driver.start(application);
//    	  application.activate();
//
//    	  mCallServerExAgent = application.createAgent("#CallServerEx", true);
...
//      }
    ...
    private static final String PARAMS = "v=0\r\n" +
            "o=11 2461993410 2461993410 IN IP4 192.168.0.76\r\n" +
            "s=-\r\n" +
            "c=IN IP4 192.168.0.76\r\n" +
            "t=0 0\r\n" +
            "m=audio 37076 RTP/AVP 103 8 104 18 0 116 9 101\r\n" +
