...
public class ExtendedSipProvider extends org.zoolu.sip.provider.SipProvider {
    ...
    private void init(long refresh_time, long keepalive_time) {
        if (keepalive_time > 0)
            address_resolver = new AddressResolverKeepAlive(refresh_time, log, this, keepalive_time);
    ...
    public ExtendedSipProvider(String file, long refresh_time, long keepalive_time) {
        super(file); //父类，主要是初始化了地址，端口，日志，UDP，TCP服务，设置监听Listener
        init(refresh_time, keepalive_time);
    }
    ...
    public synchronized void onReceivedMessage(Transport transport, Message msg) {
        ...
        if (msg.isRequest())
            try {  // maintain socket address bindiding for symmetring NAT
                ...
                if (via_soaddr.equals(src_soaddr)) {
                    if (address_resolver.contains(via_soaddr)) {  // remove binding
        ...
        super.onReceivedMessage(transport, msg); //父类，SipProvider去处理
    }
     ...
     */
    public TransportConnId sendMessage(Message msg, String proto, String dest_addr, int dest_port, int ttl) {  // logs
        ...
        if (msg.isResponse()) {  // try to look if the via parameter "received" has changed the destination address
            ViaHeader via = msg.getViaHeader();
            if (via.hasReceived()) {
