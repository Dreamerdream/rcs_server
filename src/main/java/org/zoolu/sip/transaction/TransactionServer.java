  ...
  //      printLog("new transaction-id: " + transaction_id.toString(), Log.LEVEL_HIGH);
    ...
    public void onReceivedMessage(SipProvider provider, Message msg) {
        if (msg.isRequest()) {
            ...
            if (statusIs(STATE_PROCEEDING) || statusIs(STATE_COMPLETED)) {  // retransmission of the last response
    ...
    protected void printLog(String str, int level) {
        if (log != null)
            log.println("TransactionServer#" + transaction_sqn + ": " + str, Transaction.LOG_OFFSET + level);
    }
