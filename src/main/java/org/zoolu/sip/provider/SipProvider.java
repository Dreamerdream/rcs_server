...
public class SipProvider implements Configurable, TransportListener {
    ...
    public static final SipId INVITE = new SipId(SipMethods.INVITE);
    public static final SipId REGISTER = new SipId(SipMethods.REGISTER);
    public static final SipId SUBSCRIBE = new SipId(SipMethods.SUBSCRIBE);
    public static final SipId MESSAGE = new SipId(SipMethods.MESSAGE);
    public static final SipId OPTIONS = new SipId(SipMethods.OPTIONS);
    /**
   ...
   /*protected SipProvider()
   {
    ...
    public SipProvider(String file) {
        ...
        init(via_addr, host_port); //初始化地址，端口
        initLog(); //初始化日志
        initSipTrasport(transport_protocols, transport_ports, host_ifaddr); //初始化UDP，TCP服务
    }
    ...
    private void initSipTrasport(String[] transport_protocols, int[] transport_ports, String ifaddr) {
        ...
        if (ifaddr != null && !ifaddr.equalsIgnoreCase(ALL_INTERFACES)) {
            try {
                host_ipaddr = IpAddress.getByName(ifaddr); //如果自己设置IP
            } catch (IOException e) {
                e.printStackTrace();
                host_ipaddr = null;
            }
        ...
        for (int i = 0; i < transport_protocols.length; i++) {
            try {
                ...
                if (transp != null) {
                    setTransport(transp); //本身传入Transport，回调
                }
            } catch (Exception e) {
    ...
    public void parseLine(String line) {
        ...
        if (attribute.equals("transport_ports")) {
            transport_ports = par.getIntArray();
            return;
        }
        ...
        if (attribute.equals("outbound_proxy")) {
            ...
            if (url == null || url.length() == 0 || url.equalsIgnoreCase(Configure.NONE) || url.equalsIgnoreCase("NO-OUTBOUND"))
                outbound_proxy = null;
        ...
        // certificates
        if (attribute.equals("trust_all")) {
            trust_all = (par.getString().toLowerCase().startsWith("y"));
            return;
        }
        if (attribute.equals("trust_folder")) {
            trust_folder = par.getRemainingString().trim();
            return;
        }
        if (attribute.equals("cert_file")) {
            cert_file = par.getRemainingString().trim();
            return;
        }
        if (attribute.equals("key_file")) {
            key_file = par.getRemainingString().trim();
            return;
        }
   ...
   {  via_addr=addr;
    ...
    boolean isReliableTransport(Transport transp) {
        if (transp != null) try {
            return Class.forName("org.zoolu.sip.provider.ConnectedTransport").isInstance(transp);
 ...
 //       printLog("active sip listeners: " + sip_listeners.size(), Log.LEVEL_LOW);
    ...
    public TransportConnId sendMessage(Message msg, String proto, String dest_addr, int dest_port, int ttl) {
        if (log_all_packets || msg.getLength() > MIN_MESSAGE_LENGTH)
            printLog("Resolving host address '" + dest_addr + "'", Log.LEVEL_MEDIUM);
        try {
    ...
    private TransportConnId sendMessage(Message msg, String proto, IpAddress dest_ipaddr, int dest_port, int ttl) {
        if (log_all_packets || msg.getLength() > MIN_MESSAGE_LENGTH)
            printLog("Sending message to " + (new TransportConnId(proto, dest_ipaddr, dest_port)).toString(), Log.LEVEL_MEDIUM);

        TransportConn conn = null;
        try {
            Transport transp = (Transport) sip_transports.get(proto.toLowerCase()); //map中取出来的
            if (transp != null) {
                conn = transp.sendMessage(msg, dest_ipaddr, dest_port, ttl); //调用对应的Transport的方法
            } else {
...
//        if (log_all_packets || msg.getLength() > MIN_MESSAGE_LENGTH)
//            printLog("Sending message through conn " + conn_id, Log.LEVEL_HIGH);
        printLog("message:\r\n" + msg.toString(), Log.LEVEL_LOWER);
        TransportConn conn = null;
    ...
    public synchronized void onReceivedMessage(Transport transport, Message msg) {
        try {  // logs
         //   printMessageLog(msg.getTransportProtocol(), msg.getRemoteAddress(), msg.getRemotePort(), msg.getLength(), msg, "received");
            ...
            if (msg.isRequest()) key = msg.getTransactionServerId();
            else key = msg.getTransactionClientId();
     ...
     */
    public static String pickCallId(String hostaddr) {  //String str=Long.toString(Math.abs(Random.nextLong()),16);
